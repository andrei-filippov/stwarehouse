...
export function ActPreview({ 
  ...
  const htmlContent = generateActHTML(act, pdfSettings, bankAccounts, company, true);
  ...
  const handleExportDOCX = async () => {
    try {
      await exportActToDOCX(act, pdfSettings, bankAccounts, company, true);
    } catch (error) {
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            Акт № {act.number} от {new Date(act.date).toLocaleDateString('ru-RU')}
          </span>
          {isEditing && (
        ...
        <div className="max-w-[210mm] mx-auto bg-white shadow-lg">
          {isEditing ? (
            <div
              ref={editRef}
              contentEditable
              className="border rounded-lg p-4 bg-white overflow-y-auto font-serif"
