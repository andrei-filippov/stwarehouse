// Service Worker для Yandex Object Storage (устойчивый к отсутствующим файлам)
const CACHE_NAME = 'stwarehouse-yandex-v1';
const STATIC_CACHE = 'stwarehouse-yandex-static-v1';
const ASSETS_CACHE = 'stwarehouse-yandex-assets-v1';

// Критические ресурсы для кэширования при установке
// favicon.ico убран — его может не быть
const PRECACHE_URLS = [
  '/',
  '/index.html',
  '/manifest.json',
...
// Функция для безопасного добавления в кэш (игнорирует 404)
async function cacheSafely(cache, url) {
  try {
    ...
    if (response.status === 200) {
      ...
      console.warn('[SW] Skip caching (status', response.status, '):', url);
    }
  } catch (err) {
...
// При установке кэшируем критические ресурсы (с проверкой)
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(STATIC_CACHE).then(async (cache) => {
      // Кэшируем каждый URL отдельно — если один упадёт, остальные продолжат
